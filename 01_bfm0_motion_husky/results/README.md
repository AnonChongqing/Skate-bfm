...
## 10. 当前证据视频：方法、保留原因与结论

这里只保留四段能回答关键研究问题的视频，而不是按调参次数堆放结果。

| 视频 | 实验方法 | 为什么保留 | 实测结果与结论 |
| --- | --- | --- | --- |
| `01_best_transition.mp4` | phase controller 在 push/steer reward latent 之间插值，只有 push-to-steer transition 使用 BFM tracking latent。 | 验证 transition 控制链真实执行且机器人仍直立，同时完整暴露接触间歇和人板分离。 | 180 步；最低根高 0.706 m；平均板速 +0.193 m/s；接触率 24.4%；最终距离 0.680 m。能执行 transition，不等于稳定完成。 |
| `02_safe_reactive.mp4` | 依据脚板/脚地接触、板速、人板距离和根高度，在 `push_hold` 与 `push_drive` 之间实时切换 latent。 | 这是保守闭环基线，说明物理门控能避免不满足条件时强行 transition。 | 190 步；最低根高 0.665 m；平均板速 -0.129 m/s；接触率 38.4%；最终距离 0.927 m。安全性提高，但没有有效推进。 |
| `03_kickaway_failure.mp4` | 使用较强 reverse locomotion prompt 推板。 | 负样本直接证明“板速高”可能只是把板踢走，不能作为滑板成功指标。 | 190 步；最低根高 0.111 m；平均板速 +0.760 m/s；接触率仅 2.6%。速度奖励必须受 retention、接触和 upright 约束。 |
| `04_direct_steer.mp4` | CEM 适配静态 BFM latent，混入 signed rotate latent，并以脚到板 marker 误差选择 BFM recovery latent。 | 这是 frozen-BFM direct steer 的最佳证据，也展示手工 latent 闭环的性能上限；视频保留后半段失败。 | 首次完全脱板由第 35 步推迟到第 47 步，最终距离由 0.914 m 降到 0.574 m；第 58 步后仍持续脱板。 |

四段视频共同说明：BFM0 能在 HUSKY 中生成有意义的 push、transition 和短时
steer 动作，但静态 prompt、CEM 和少数手选反馈方向无法覆盖滑板接触变化。
这也是 Stage 03 引入 state-conditioned latent flow 与 skateboard Twin Q 的直接原因。
